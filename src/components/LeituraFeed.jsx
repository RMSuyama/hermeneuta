...
const LeituraFeed = ({ leituras }) => {
  ...
  const handleShareBook = (e, book) => {
    e.stopPropagation();
    const text = `Confira o livro "${book.title}" por ${book.author} no Hermeneuta!`;
    if (navigator.share) {
      navigator.share({
        title: book.title,
        text: text,
      ...
      navigator.clipboard.writeText(`${text} ${book.link || window.location.href}`);
      alert('Indicação copiada!');
    }
  };

  if (!leituras || leituras.length === 0) return <div className="leitura-feed">Nenhuma indicação de leitura.</div>;

  const categories = ['TODOS', ...new Set(leituras.map(b => b.category).filter(Boolean))];

  const filteredBooks = activeCategory === 'TODOS'
    ? leituras
    : leituras.filter(b => b.category === activeCategory);
    ...
    <div className="leitura-feed">
      <div className="section-header">
        ...
        <div className="header-actions">
          <p>Sugestões de leitura para aprofundamento jurídico, técnico e cultural.</p>
          ...
          >
            ...
            <div className="book-info">
              ...
              <div className="book-actions">
                ...
                <button className="share-btn-card" onClick={(e) => handleShareBook(e, book)} title="Compartilhar">
                  <Share2 size={16} />
                </button>
                {book.link && book.link !== '#' && (
